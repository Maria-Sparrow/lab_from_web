...
function Cards() {
    ...
    const [search, setCardSearched] = useState(item);
    const [filter, setCardFilter] = useState(item);

    const [searchDone, setSearch] = useState(false);
    const [filterDone, setFilter] = useState(false);

    function Searched(searchItem){
        if (searchItem !== null && searchItem !== ""){
            let arr = []
            if (filterDone && searchDone){
                arr = filter.filter(value => search.includes(value));
            } else if (filterDone){
                arr = filter;
            }else if(searchDone) {
                arr = search;
            ...
            setCardSearched(arr.filter(items => items.name === searchItem));
